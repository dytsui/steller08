  ...
  return (
    <form className="stack" onSubmit={onSubmit}>
      <div className="pill-row">
        <button
          type="button"
          className={`button ${role === "user" ? "button-primary" : "button-neutral"}`}
        ...
        <button
          type="button"
          className={`button ${role === "pro" ? "button-primary" : "button-neutral"}`}
          onClick={() => setRole("pro")}
        >
          Pro端
        </button>
      ...
      <label className="stack">
        ...
        <input
          ...
          placeholder={role === "pro" ? "教练 / 工作室名称" : "你的名称"}
        />
      </label>
      ...
      <Button tone="primary" disabled={pending}>
        {pending ? "创建中..." : `创建${role === "pro" ? "Pro端" : "用户端"}账号`}
      </Button>
    </form>
  );
}
